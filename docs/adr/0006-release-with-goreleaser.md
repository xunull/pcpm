...
- Publishing to the tap needs a token with `contents: write` on `xunull/homebrew-tap`; the repository's own `GITHUB_TOKEN` cannot write to another repository. The release job therefore carries two tokens with least privilege: `GITHUB_TOKEN` creates the Release, `TAP_PUSH_TOKEN` only pushes the formula. A missing `TAP_PUSH_TOKEN` fails the release loudly rather than silently skipping.
